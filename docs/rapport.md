...
### 5.1 Authentification MQTT

Mosquitto est configuré avec `allow_anonymous false`. Deux comptes sont définis :

- `esp32` : publication/subscription côté firmware ;
- `nodered` : ingest et supervision côté Node-RED.

Les mots de passe ne sont jamais commités en clair dans le dépôt.

### 5.2 Validation des JSON entrants

Tout JSON reçu (MQTT cmd, API POST) passe par `security.cpp` :

- Vérification anti-overflow ArduinoJson.
- **Commandes actionneurs** : champ `action` obligatoire, valeur dans une liste blanche (`led_on`, `led_off`, `relay_on`, `relay_off`).
- **Config MQTT** : broker non vide, port 1–65535, champs user/password sans caractères de contrôle.

Côté Node-RED, les messages `data` non conformes sont rejetés et signalés sur le topic `alerts`.

### 5.3 Protection de l’API locale

Les routes POST peuvent exiger un en-tête `X-Api-Token` si `API_TOKEN` est défini dans `secrets.h`. Par défaut (token vide), l’API reste ouverte — acceptable en environnement de TP sur réseau local, à durcir en production.

### 5.4 Gestion des secrets

| Secret | Emplacement | Versionné |
|--------|-------------|-----------|
| WiFi SSID/mot de passe | `secrets.h` (copie de `.example`) | Non |
| MQTT broker/user/password (défaut) | `secrets.h` | Non |
| Token API | `secrets.h` | Non |
| Mots de passe Docker | `docker/.env` | Non |
| Hash Mosquitto | `docker/mosquitto/config/passwd` | Oui (hash) |

La configuration MQTT modifiable via le web est persistée en **NVS** (namespace `iotcfg`), permettant un changement de broker sans reflash.

---

## 6. Observabilité et performances

### 6.1 Métriques firmware

La tâche `supervision` log toutes les 10 secondes :

```
[supervision] uptime=3600s heap_free=198432 bytes mqtt=1 latence=45ms
```

Ces valeurs sont aussi exposées via `/api/status` pour le dashboard web.

### 6.2 Latence de publication

Mesurée à chaque `publish()` MQTT (temps entre l’appel et le retour de la librairie). Typiquement < 100 ms sur réseau local.

### 6.3 Consommation mémoire

Piles FreeRTOS dimensionnées conservativement (web : 10 Ko pour le parsing JSON et le serveur HTTP). Le heap libre reste stable autour de 180–220 Ko en fonctionnement nominal, loin de la limite critique de l’ESP32 (~300 Ko disponibles).

### 6.4 Badges obtenus

| Badge | Critère | Statut |
|-------|---------|--------|
| Sensor Engineer | Acquisition fiable + filtrage + détection aberrants | ✅ |
| Network Engineer | MQTT robuste, QoS 1, reconnexion | ✅ |
| Embedded Architect | Multitâche propre, pas de delay | ✅ |
| Security Engineer | Auth + validation JSON + API protégée | ✅ |
| Full-Stack IoT | Web embarquée + Node-RED + commandes (web / MQTT) | ✅ |
| Reliability Engineer | Buffer offline + rejeu | ✅ |
| Performance Engineer | Heap/uptime/latence | ✅ |

---

## 7. Tests et validation

### 7.1 Scénarios exécutés

| Scénario | Procédure | Résultat attendu |
|----------|-----------|------------------|
| Publication nominale | ESP32 connecté, broker up | Jauges et courbes Node-RED + points InfluxDB |
| Coupure MQTT | `docker compose stop mosquitto` | Bufferisation LittleFS, pas de crash |
| Reconnexion | `docker compose start mosquitto` | Rejeu FIFO, timestamps d’origine conservés |
| JSON invalide | `./docker/test/simulate-esp32.sh invalid` | Rejet + alerte sur topic `alerts` |
| Commande actionneur | Interface web embarquée ou publish MQTT `cmd` | LED/relais changent d’état sur l’ESP32 |
| Config MQTT web | Modification broker via interface | Reconnexion avec nouveaux paramètres NVS |
| Mode AP | WiFi routeur indisponible | Accès web via `esp32-esp32-1` |

### 7.2 Outils de test

- **Simulateur shell** (`docker/test/simulate-esp32.sh`) : publie des messages valides/invalides sans matériel.
- **Requête InfluxDB** : vérification du stockage avec le timestamp `ts` du message, pas l’heure d’arrivée.
- **Moniteur série** (`pio device monitor`) : traces structurées par module (`[sensor]`, `[network]`, `[storage]`, etc.).
...
- **Capteur environnement** : DHT22 utilisé à la place du BME280 imposé — même rôle fonctionnel, broches différentes à adapter si changement matériel.
- **Champs offline** : le buffer JSONL ne stocke que `temp` et `humidity` (format minimal imposé) ; les champs `potentiometer` et `button` ne sont persistés qu’en publication directe.
- **Sécurité web** : pas de TLS sur le serveur HTTP local (contrainte matérielle et périmètre TP).
- **Historique Node-RED** : les courbes `ui_chart` affichent le flux live (1 h glissante), pas une requête InfluxDB — l’historique long terme reste consultable via InfluxDB ou Grafana.
- **Commandes supervision** : plus de boutons dans le dashboard Node-RED ; le pilotage passe par l’interface web ESP32.
- **Bonus Grafana** : non implémenté — InfluxDB 1.x est prêt comme source de données.

### 8.2 Pistes d’évolution

1. Ajouter Grafana au `docker-compose.yml` avec alertes (absence de données, seuil température), en s’appuyant sur l’historique InfluxDB déjà alimenté.
2. Migrer vers un capteur I²C (BME280) pour réduire la latence de lecture et la consommation.
3. Chiffrer les communications MQTT (TLS/MQTTS) et activer le token API par défaut.
4. Réintroduire des boutons de commande dans Node-RED ou alimenter les graphiques depuis InfluxDB pour un historique plus long que la fenêtre live.

---

## 9. Conclusion

Le projet **IoT Secure Station** livre une chaîne bout en bout fonctionnelle : acquisition capteurs filtrée et horodatée, publication MQTT résiliente avec mode offline, interface web de configuration et de contrôle, supervision Node-RED avec jauges, courbes historiques et historisation InfluxDB.

L’architecture respecte les contraintes du cahier des charges : code modulaire, multitâche FreeRTOS sans `delay()`, validation des entrées, authentification MQTT et observabilité des ressources. Les sept badges principaux sont couverts ; seul le bonus Grafana reste ouvert.

Le dépôt (`iot-secure-station/`) contient l’ensemble du code source, du flux Node-RED, de la stack Docker et de la documentation associée, prêt pour la démonstration finale.
...
- Spécification : `Système Iot sécurisé et autonome.pdf`
- Architecture : `docs/architecture.drawio` · capture : `docs/screenshots/architecture.png`
- Suivi infra : `docs/suivi-projet.md`
